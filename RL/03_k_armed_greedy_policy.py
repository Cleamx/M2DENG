# Implementation of a k-armed bandit problem using a greedy action selection policy.
...
def run_xp():
    ...
    arm_cumulative_reward = np.zeros(k)
    ...
    for t in range(1, num_steps + 1):
        ...
        N[selected_arm] += 1
        arm_cumulative_reward[selected_arm] += reward
        Q[selected_arm] = arm_cumulative_reward[selected_arm]/N[selected_arm]
...
for run in range(num_runs):
    rewards_mean, optimal = run_xp()
    all_rewards[run] = rewards_mean
...
ax2.set_ylim(0, 100)
#fig.tight_layout()
plt.title(f'Greedy policy on {k}-armed Bandit for {num_steps} steps\n averaged over {num_runs} runs with initial seed {experiment_seed}')
